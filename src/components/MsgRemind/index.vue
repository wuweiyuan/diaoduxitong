<template>
	<div>
		<el-badge :value="msgMind" style="line-height: 30px;">
			<el-button size="small" type="primary" @click="jump">
...
import {
    page
  } from "api/admin/msgRemind/index";
  export default {
    name: 'errLog',
    props: {
      logsList: {
      ...
      return {
		...
		methods:{
			jump(){
				...
				this.$router.push({ name: '生产单查询', params: { stateCode: 1 }})
			},
			upload(){
				page().then(res=>{
					if(res.status==200){
